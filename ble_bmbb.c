 ...
 */
static uint32_t rx_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init, uint8_t *init_val, uint16_t init_val_size)
{
    ...
    ble_uuid.uuid = BLE_UUID_BMBB_RX_CHARACTERISTIC;
    ...
    attr_char_value.init_len  = init_val_size;
    attr_char_value.init_offs = 0;
    attr_char_value.max_len   = init_val_size;
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init, uint8_t *init_val, uint16_t init_val_size)
{
    uint32_t      err_code;
    ble_uuid_t    ble_uuid;
    ble_uuid128_t nus_base_uuid = BMBB_BASE_UUID;
...
uint32_t ble_nus_string_set(ble_nus_t * p_nus, uint8_t * p_string, uint16_t length)
{
    ...
    return sd_ble_gatts_value_set(p_nus->conn_handle, p_nus->rx_handles.value_handle, &gatts_value);
}
